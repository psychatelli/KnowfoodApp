...
export const AuthorizedUser = () => async dispatch => {
    ...
    try{
        ...
        const res = await axios.post('http://192.168.254.16:5000/api/auth/getuser', token, config);
...
export const loadUser = (token) => async dispatch => {
    ...
    try{
        const res = await axios.get('http://192.168.254.16:5000/api/auth');
...
export const register = ({username, email, password }) => async dispatch => {
    ...
    try{
        const res = await axios.post('http://192.168.254.16:5000/api/users', body, config);
        dispatch({
...
export const login = ({email, password}) => async dispatch => {
    ...
    try{
        const res = await axios.post('http://192.168.254.16:5000/api/auth', body, config);
